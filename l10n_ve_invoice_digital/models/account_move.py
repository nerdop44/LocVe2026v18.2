...
class AccountMove(models.Model):
    ...
    ocultar_forma_libre_digital = fields.Boolean(related="company_id.ocultar_forma_libre_digital", string="Ocultar Forma Libre")

    def action_open_void_control_wizard(self):
        if self.company_id.ocultar_forma_libre_digital:
            raise UserError(_(
                "El control de correlativos por atasco está deshabilitado. "
                "Use facturación digital (TFHKA) o Máquina Fiscal."
            ))
        return super().action_open_void_control_wizard()
    ...
    def get_payment_methods(self):
        total_ves = self.amount_total or 0.0
        if total_ves <= 0:
            return False
        rate = self.tax_today or 1.0
        is_usd = self.currency_id.name == 'USD'
        if is_usd and rate > 0:
            total_ves = total_ves * rate
        forma = "01"
        if self.invoice_payment_term_id:
            term_name = (self.invoice_payment_term_id.name or "").lower()
            if "tarjeta" in term_name or "tdd" in term_name or "tdc" in term_name:
                forma = "02"
            elif "transferencia" in term_name or "banco" in term_name:
                forma = "01"
            elif "efectivo" in term_name or "pago móvil" in term_name:
                forma = "04"
        date_str = (self.invoice_date or fields.Date.today()).strftime("%d/%m/%Y")
        return [{
