...
from ...tools import locve_cne_query  # Módulo de consulta al CNE venezolano
...
class ResPartner(models.Model):
    ...
    def create(self, vals_tree):
        ...
        calling the function get_default_name_by_vat from locve_cne_query before create the partner
        ...
        for vals in vals_tree:
            if vals.get("vat") and not vals.get("name", False):
                ...
                if prefix_vat == "V" and not name and prefix_vat in ["V", "E"]:
                    # Consulta CNE controlada por parámetro de configuración
                    cne_enabled = self.env['ir.config_parameter'].sudo().get_param(
                        'l10n_ve_contact.cne_query_enabled', default='False'
                    ) == 'True'
                    if cne_enabled:
                        name, flag = locve_cne_query.get_default_name_by_vat(self, prefix_vat, vat)
                        if flag:
                            vals["name"] = name
    ...
    # def _onchange_(self):
    #     """This function assign the name of the person by the vat number and the prefix of the vat number
    #     calling the function get_default_name_by_vat from locve_cne_query
    ...
    #         self._check_vat()
    #         name, flag = locve_cne_query.get_default_name_by_vat(self, self.prefix_vat, self.vat)
    #         if not flag:
    #             return
    #         for record in self:
    #             record.name = name
