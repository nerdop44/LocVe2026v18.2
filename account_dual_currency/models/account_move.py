...
class AccountMove(models.Model):
    ...
    tax_today = fields.Float(string="Tasa de Factura", store=True, readonly=False, digits=(16, 4),
    ...
    # Community refactor: campos de referencia de activos fijos (dependen de account_asset, Enterprise)
    ...
    # Community refactor: depreciation_value_ref depende de account_asset (Enterprise)
    ...
    # Community refactor: toda la lógica de activos fijos (account.asset, Enterprise) se
    # desactiva. El núcleo de doble moneda (balance_usd, tax_today, currency_id_dif, etc.)
    # se conserva; solo se pierde el reporte de depreciación en doble moneda.
    ...
    def _compute_tax_today(self):
        for rec in self:
            ...
            rec.tax_today = round(rate_val, 4) if rate_val > 1.0 else 1.0
        ...
        for rec in self:
            if not rec.tax_today_edited:
                ...
                if new_rate_ids and rec.company_id.currency_id_dif.id in new_rate_ids:
                    rec.tax_today = round(1.0 / new_rate_ids[rec.company_id.currency_id_dif.id], 4)
                else:
                    ...
                    if module_dual_currency:
                        ...
                        if move_currency_id and currency_dif and move_currency_id == currency_dif.id:
                            ...
                            if 0.0 < db_rate < 1.0:
                                ...
                                new_rate = db_rate
                            # Fallback: si la tasa es muy baja (<10) usar tasa actual
                            if new_rate <= 10.0:
                                new_rate = currency_dif.get_trm_systray() if currency_dif else 1.0
                            val.update({'tax_today': round(new_rate, 4)})
    ...
    def write(self, vals):
        ...
        if new_date:
            for rec in self:
                ...
                if new_rate and new_rate > 0:
                    vals['tax_today'] = round(new_rate, 4)
        ...
        if tax_today is not None and tax_today > 1.0:
            ...
            pass  # No removal - let the calculated rate pass through
    ...
    def _onchange_invoice_date_or_date(self):
        for rec in self:
            ...
            if rec.company_id.currency_id_dif and not rec.tax_today_edited and not getattr(rec, 'manually_set_rate', False):
                ...
                # Fallback: si la tasa es muy baja (<10) probablemente es una tasa obsoleta de 2010
                # Usar la tasa actual del BCV en su lugar
