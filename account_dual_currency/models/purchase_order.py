...
class PurchaseOrder(models.Model):
    ...
    def action_create_invoice(self):
        ...
        for order in self:
            ...
            if 'invoice_line_ids' not in invoice_vals or not isinstance(invoice_vals['invoice_line_ids'], list):
        ...
        for grouping_keys, invoices in groupby(invoice_vals_list, key=lambda x: (x.get('company_id'), x.get('partner_id'), x.get('currency_id'))):
            ...
            for invoice_vals in invoices:
                if not ref_invoice_vals:
                    ...
                    if 'invoice_line_ids' not in ref_invoice_vals or not isinstance(ref_invoice_vals['invoice_line_ids'], list):
                        ref_invoice_vals['invoice_line_ids'] = []
                else:
                    if 'invoice_line_ids' not in ref_invoice_vals or not isinstance(ref_invoice_vals['invoice_line_ids'], list):
                        ref_invoice_vals['invoice_line_ids'] = []
                    lines_to_add = invoice_vals.get('invoice_line_ids', [])
                    if isinstance(lines_to_add, list):
                        ref_invoice_vals['invoice_line_ids'] += lines_to_add
