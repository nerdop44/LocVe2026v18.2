...
class SaleOrder(models.Model):
    ...
    def _compute_amount_total_dif(self):
        for record in self:
            date_to_use = fields.Date.to_date(record.date_order) if record.date_order else fields.Date.today()
            dif = record.currency_id_dif or record.company_id.currency_id_dif
            if not dif:
            ...
            if src == dif:
                ...
                record.amount_total_dif = src._convert(record.amount_total, dif, company, date_to_use, round=True)
                record.amount_untaxed_dif = src._convert(record.amount_untaxed, dif, company, date_to_use, round=True)
                record.amount_tax_dif = src._convert(record.amount_tax, dif, company, date_to_use, round=True)
