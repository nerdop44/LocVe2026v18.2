...
class ResCurrencyRate(models.Model):
    ...
    def compute_rate(self, foreign_currency_id, rate_date):
        ...
        if not rates:
            ...
            if self.env.company.currency_id.name == 'USD':
                return {
                    "foreign_rate": round(currency.rate, 4) if currency.rate else 1.0,
                    "foreign_inverse_rate": round(currency.inverse_rate, 4) if currency.inverse_rate else 1.0,
                }
            else:
                return {
        ...
        if company_currency.name == 'USD':
            return {
                "foreign_rate": round(rate.company_rate, 4),
                "foreign_inverse_rate": round(rate.inverse_company_rate, 4),
            }
        else:
            return {
                "foreign_rate": round(rate.inverse_company_rate, 4),
                "foreign_inverse_rate": round(rate.company_rate, 4),
            }
    ...
    def compute_inverse_rate(self, rate):
        ...
        inverse_rate = round(1.0 / rate, 4) if foreign_currency_id == base_usd_id else round(rate, 4)
        return inverse_rate
